...
int main(void) {
    ...
    UA_Variant_setScalarCopy(&input[1], &iVariant, &UA_TYPES[UA_TYPES_STRING]);
    UA_Client_call_async(client, UA_NODEID_NUMERIC(0, UA_NS0ID_OBJECTSFOLDER), UA_NODEID_STRING(1, "handleClientData"), 1, input, methodCalled, NULL, &reqId);
    */
    //UA_Client_run_iterate(client, 0);
    //UA_Client_run_iterate(client, 0);
    ...
    if(ss == UA_SESSIONSTATE_ACTIVATED) {
            ...
            UA_Client_writeValueAttribute_async(client, UA_NODEID_STRING(1, "the.answer"), &iVariant, attrWritten, NULL, &reqId);
            UA_Variant_clear(&iVariant);
